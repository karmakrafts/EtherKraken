...
KRAKEN_EXPORT kraken_error_t kraken_clock_create(const double frequency, kraken_clock_handle_t* handle) {
    ...
    kraken_clock_t* clock = kraken_calloc(sizeof(kraken_clock_t));
    ...
    KRAKEN_CHECK_CALL_ERR(kraken_mutex_create(&clock->drivers_mutex), "Could not initialize clock driver list mutex");
...
KRAKEN_EXPORT kraken_error_t kraken_clock_register(kraken_clock_handle_t handle, kraken_driver_handle_t driver) {
    ...
    atomic_store(&handle->next_event, now - handle->period);

    kraken_mutex_lock(&handle->drivers_mutex);
    KRAKEN_CHECK_CALL_ERR(kraken_array_list_add(&handle->drivers, &driver),
                          "Could not add driver to clock drivers list");
    kraken_mutex_unlock(&handle->drivers_mutex);
...
KRAKEN_EXPORT kraken_error_t kraken_clock_destroy(kraken_clock_handle_t handle) {
    ...
    KRAKEN_CHECK_CALL_RES(kraken_mutex_destroy(&handle->drivers_mutex), KRAKEN_ERR_INVALID_OP,
...
kraken_error_t kraken_clock_tick(kraken_clock_t* clock) {
    kraken_mutex_lock_no_yield(&clock->drivers_mutex);
    ...
    kraken_mutex_unlock(&clock->drivers_mutex);
    return KRAKEN_OK;
}