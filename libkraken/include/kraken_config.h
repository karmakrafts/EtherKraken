...
typedef kraken_bool_t (*pfn_kraken_i2c_mux_state_get)(int fd, const kraken_pin_config_t* pin);
typedef void (*pfn_kraken_i2c_mux_state_set)(int fd, kraken_bool_t state, const kraken_pin_config_t* pin);

typedef struct kraken_i2c_mux_config {
    kraken_mux_type_t type;
    const kraken_pin_config_t* pins;
    size_t pin_count;
    pfn_kraken_i2c_mux_state_get pfn_state_get;
    pfn_kraken_i2c_mux_state_set pfn_state_set;
    const char* bus;
    kraken_i2c_address_t address;
} kraken_i2c_mux_config_t;

typedef kraken_bool_t (*pfn_kraken_spi_mux_state_get)(const void* base_address, const kraken_pin_config_t* pin);
typedef void (*pfn_kraken_spi_mux_state_set)(void* base_address, kraken_bool_t state, const kraken_pin_config_t* pin);

typedef struct kraken_spi_mux_config {
    kraken_mux_type_t type;
    const kraken_pin_config_t* pins;
    size_t pin_count;
    pfn_kraken_spi_mux_state_get pfn_state_get;
    pfn_kraken_spi_mux_state_set pfn_state_set;
